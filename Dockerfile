...
ENV FS_ZIP_FILE=fscrawler-es7-2.7-20210121.084108-154.zip
...
COPY config/test3 /usr/share/fscrawler/config-mount/test/
COPY _settings.json /usr/share/fscrawler/config/_default/7/
COPY entry.sh /
...
CMD ["fscrawler", "--trace", "--rest", "--config_dir", "/usr/share/fscrawler/config", "test"]
